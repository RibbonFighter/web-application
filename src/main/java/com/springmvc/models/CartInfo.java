...
public class CartInfo {
	...
	public void addProduct(ProductInfo productInfo, int quantity) {
		...
		if (cartLineInfo == null) {
			...
			this.cartLineInfos.add(cartLineInfo); //lay cartForm.cartLineInfos
		}
	...
	public int getQuantityTotal() {
		int quantity = 0;
		for (CartLineInfo cartLineInfo : this.cartLineInfos) {
			quantity += cartLineInfo.getQuantity(); //lay so luong hang da mua + lai
		}
		return quantity;
	}
	...
	public double getAmountTotal() {
		double total = 0;
		for (CartLineInfo cartLineInfo : this.cartLineInfos) {
			total += cartLineInfo.getAmount(); // tong gia tri cua tat ca hang da mua + lai
		}
		return total;
	}
	
	public void updateQuantity(CartInfo cartForm) {
		if (cartForm != null) { //lay qauntity ben jsp truyen qua update lai moi lan add them product 3 5
